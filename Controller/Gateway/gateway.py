...
NOTAS_SERVICE = "http://localhost:5005"
    ...
    if request.method == "GET":
        ...
        try:
            ...
            response = requests.post(
                f"{CADASTRO_SERVICE}/cadastro", json=data, timeout=5
            )
...
def login():
    ...
    if request.method == "GET":
        ...
        try:
            ...
            response = requests.post(f"{LOGIN_SERVICE}/login", json=data, timeout=5)
...
def api_cadastro():
    ...
    try:
        ...
        response = requests.post(f"{CADASTRO_SERVICE}/cadastro", json=data, timeout=5)
        return jsonify(response.json()), response.status_code
    except requests.exceptions.ConnectionError:
        return jsonify({"error": "Serviço de cadastro indisponível"}), 503
    except Exception as e:
...
def api_registro_turma():
    ...
    try:
        data = request.get_json()
        response = requests.post(
            f"{CLASSES_SERVICE}/registro_turma", json=data, timeout=5
        )
        return jsonify(response.json()), response.status_code
    except requests.exceptions.ConnectionError:
...
def api_adicionar_aluno_turma():
    ...
    try:
        data = request.get_json()
        response = requests.post(
            f"{CLASSES_SERVICE}/adicionar_aluno_turma", json=data, timeout=5
        )
        return jsonify(response.json()), response.status_code
    except requests.exceptions.ConnectionError:
        return jsonify({"error": "Serviço de turma indisponível"}), 503
...
def api_buscar_usuario():
    ...
    try:
        ...
        response = requests.get(
            f"{CLASSES_SERVICE}/buscar_usuario", params={"email": email}, timeout=5
        )
        return jsonify(response.json()), response.status_code
    except requests.exceptions.ConnectionError:
        return jsonify({"error": "Serviço de turma indisponível"}), 503
...
def api_iniciar_aula():
    ...
    try:
        data = request.get_json()
        response = requests.post(
            f"{CLASSES_SERVICE}/iniciar_aula", json=data, timeout=5
        )
        return jsonify(response.json()), response.status_code
    except requests.exceptions.ConnectionError:
        return jsonify({"error": "Serviço de turma indisponível"}), 503
...
def api_registrar_presenca_aula():
    ...
    try:
        data = request.get_json()
        response = requests.post(
            f"{CLASSES_SERVICE}/registrar_presenca_aula", json=data, timeout=5
        )
        return jsonify(response.json()), response.status_code
    except requests.exceptions.ConnectionError:
        return jsonify({"error": "Serviço de turma indisponível"}), 503
...
def api_terminar_aula():
    ...
    try:
        data = request.get_json()
        response = requests.post(
            f"{CLASSES_SERVICE}/terminar_aula", json=data, timeout=5
        )
        return jsonify(response.json()), response.status_code
    except requests.exceptions.ConnectionError:
        return jsonify({"error": "Serviço de turma indisponível"}), 503
...
def api_timer_registrar_presenca():
    try:
        data = request.get_json()
        response = requests.post(
            f"{TIMER_SERVICE}/timer/registrar_presenca", json=data, timeout=5
        )
        return jsonify(response.json()), response.status_code
    except requests.exceptions.ConnectionError:
        return jsonify({"error": "Serviço de timer indisponível"}), 503
...
def api_timer_encerrar():
    try:
        data = request.get_json()
        response = requests.post(
            f"{TIMER_SERVICE}/timer/encerrar", json=data, timeout=5
        )
        return jsonify(response.json()), response.status_code
    except requests.exceptions.ConnectionError:
        return jsonify({"error": "Serviço de timer indisponível"}), 503
...
@app.route("/api/notas/publicar", methods=["POST"])
def api_notas_publicar():
    try:
        data = request.get_json()
        response = requests.post(f"{NOTAS_SERVICE}/notas/publicar", json=data, timeout=5)
        return jsonify(response.json()), response.status_code
    except requests.exceptions.ConnectionError:
        return jsonify({"error": "Serviço de notas indisponível"}), 503
    except Exception as e:
        return jsonify({"error": str(e)}), 500


@app.route("/api/notas/listar", methods=["GET"])
def api_notas_listar():
    try:
        params = {}
        for key in ("turma_id", "aluno_email", "aula_id"):
            val = request.args.get(key)
            if val:
                params[key] = val
        response = requests.get(f"{NOTAS_SERVICE}/notas/listar", params=params, timeout=5)
        return jsonify(response.json()), response.status_code
    except requests.exceptions.ConnectionError:
        return jsonify({"error": "Serviço de notas indisponível"}), 503
    except Exception as e:
...
@app.route("/api/notas/aluno", methods=["GET"])
def api_notas_aluno():
    try:
        email = request.args.get("email", "")
        response = requests.get(
            f"{NOTAS_SERVICE}/notas/aluno", params={"email": email}, timeout=5
        )
        return jsonify(response.json()), response.status_code
    except requests.exceptions.ConnectionError:
        return jsonify({"error": "Serviço de notas indisponível"}), 503
...
@app.route("/api/notas/editar", methods=["PUT"])
def api_notas_editar():
    try:
        data = request.get_json()
        response = requests.put(f"{NOTAS_SERVICE}/notas/editar", json=data, timeout=5)
        return jsonify(response.json()), response.status_code
    except requests.exceptions.ConnectionError:
        return jsonify({"error": "Serviço de notas indisponível"}), 503
    except Exception as e:
...
@app.route("/api/notas/remover", methods=["DELETE"])
def api_notas_remover():
    try:
        data = request.get_json()
        response = requests.delete(f"{NOTAS_SERVICE}/notas/remover", json=data, timeout=5)
        return jsonify(response.json()), response.status_code
    except requests.exceptions.ConnectionError:
        return jsonify({"error": "Serviço de notas indisponível"}), 503
    except Exception as e:
